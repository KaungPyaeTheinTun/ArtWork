...
function TestimonialCard({ test }: { test: typeof testimonials[0] }) {
  ...
  return (
    <div
      ref={cardRef as any}
      className="relative bg-[#141414] border border-white/10 rounded-[8px] p-8 md:p-10 overflow-hidden flex flex-col justify-between min-h-[280px] group transition-all duration-700 opacity-0 translate-y-[60px] [&.revealed]:opacity-100 [&.revealed]:translate-y-0 hover:-translate-y-2 hover:shadow-[0_20px_60px_rgba(0,0,0,0.4)]"
      style={{ transitionDelay: `${test.delay}ms` }}
    >
      {/* Background Art */}
      <div
        className="absolute inset-0 z-0 transition-opacity duration-500 group-hover:opacity-[0.25]"
        style={{
...
export default function TestimonialsSection() {
  ...
  return (
    <section
      ...
      className="py-[120px] px-6 md:px-12 w-full max-w-7xl mx-auto"
    >
      {/* Header — words animate once section has revealed class */}
      <h2 className="font-bold text-[clamp(32px,5vw,64px)] text-[#f5f0eb] mb-16 flex flex-wrap gap-x-4">
        {'they say what they feel'.split(' ').map((word, i) => (
          <span
            key={i}
            className="inline-block transition-all duration-700 opacity-0 translate-y-[20px] [.revealed_&]:opacity-100 [.revealed_&]:translate-y-0"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {testimonials.map((test) => (
          <TestimonialCard key={test.author} test={test} />
        ))}
      </div>
    </section>
  );
