...
const ALL_ARTWORKS = [
  { id: 1,  title: 'The Last Rehearsal',  medium: 'Mixed Media', year: '2023', dims: '90 × 120 cm',  image: artwork1, aspect: 'tall',   description: 'A study in tension — paint applied over found stage scripts, layered until the original text becomes a ghost beneath.' },
  { id: 2,  title: 'Midnight Script',     medium: 'Acrylic',     year: '2022', dims: '100 × 100 cm', image: artwork2, aspect: 'square', description: 'Painted in a single nocturnal session. The canvas holds the silence of a theater after everyone has gone home.' },
  { id: 3,  title: 'The Audience',        medium: 'Installation', year: '2023', dims: '200 × 500 cm', image: artwork3, aspect: 'wide',   description: 'Multi-panel installation exploring the gaze — who watches, who is seen, and what survives the looking.' },
  { id: 4,  title: 'Crimson Interior',    medium: 'Oil',          year: '2021', dims: '80 × 100 cm',  image: g1,      aspect: 'tall',   description: 'Built from the inside out — layers of cadmium red worked into raw umber until a kind of warmth emerges from darkness.' },
  { id: 5,  title: 'Torn Archive',        medium: 'Mixed Media',  year: '2022', dims: '120 × 90 cm',  image: g2,      aspect: 'wide',   description: 'Newsprint and memory. The fragments resist legibility — deliberately — so that feeling precedes reading.' },
  { id: 6,  title: 'Body in Transit',     medium: 'Oil',          year: '2023', dims: '70 × 100 cm',  image: g3,      aspect: 'tall',   description: 'A figure caught between stillness and flight. The brushwork mirrors the indecision of motion not yet committed.' },
  { id: 7,  title: 'Gray Study No. 4',    medium: 'Charcoal & Gesso', year: '2020', dims: '60 × 60 cm', image: g4,  aspect: 'square', description: 'Part of a year-long monochromatic series. Restraint as method; the single value forced every other decision.' },
  { id: 8,  title: 'Pour — Lisbon',       medium: 'Acrylic',     year: '2022', dims: '90 × 120 cm',  image: g5,      aspect: 'wide',   description: "Liquid pigment directed by gravity and breath. Control surrendered; the city's tides worked into the paint." },
  { id: 9,  title: 'Gesture Study III',   medium: 'Ink',          year: '2021', dims: '50 × 70 cm',   image: g6,      aspect: 'tall',   description: 'A choreography of marks — ink splattered first, then the slower work of interpreting the accident into intention.' },
  { id: 10, title: 'Storm Coast',         medium: 'Oil',          year: '2023', dims: '150 × 80 cm',  image: g7,      aspect: 'wide',   description: 'Atlantic weather painted from memory, three weeks after the trip. The distance made the gray more accurate.' },
  { id: 11, title: 'Trace — Movement IV', medium: 'Installation', year: '2022', dims: 'Variable',     image: g8,      aspect: 'tall',   description: 'Long-exposure performance documentation elevated to artwork. The dancer became the brush; the studio the canvas.' },
  { id: 12, title: 'Surface Memory',      medium: 'Mixed Media',  year: '2020', dims: '40 × 40 cm',   image: g9,      aspect: 'square', description: 'A micro study. The texture holds everything — scraping, repainting, scraping again — until the paint becomes sediment.' },
  { id: 13, title: 'Interval',            medium: 'Acrylic',     year: '2021', dims: '130 × 90 cm',  image: g10,     aspect: 'wide',   description: 'Two states in dialogue across a single plane. Neither resolves; the tension between them is the subject.' },
];

const FILTERS = ['All', 'Oil', 'Acrylic', 'Mixed Media', 'Installation', 'Ink', 'Charcoal & Gesso'];

/* Span classes only apply on sm+ so mobile always gets 1 column, no overflows */
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: `opacity 600ms ease ${index * 55}ms, transform 600ms cubic-bezier(0.22,1,0.36,1) ${index * 55}ms`,
      }}
      onClick={onClick}
    ...
    >
      ...
      {/* Gradient overlay — stronger on mobile so labels always readable */}
      <div
      ...
      {/* Labels — always visible on mobile, enhanced on hover/desktop */}
      <div className="absolute inset-0 flex flex-col justify-end p-4 sm:p-5">
        ...
        >
          ...
          {/* Description — only shows on hover, desktop only */}
          <p
            className="hidden sm:block text-[#8a8580] text-[12px] leading-relaxed mt-1.5 line-clamp-2 max-w-[280px]"
            style={{ opacity: hovered ? 1 : 0, transition: 'opacity 250ms ease 80ms' }}
          >
            {artwork.description}
          </p>
        </div>
      ...
      >
        <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
          <path d="M2 7h10M7 2l5 5-5 5" stroke="#f5f0eb" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
    </div>
  );
      ...
      >
        ...
        <div className="relative flex-1 flex items-center justify-center p-4 sm:p-8 min-h-0">
          <img
            ...
            style={{ animation: 'lightboxIn 350ms cubic-bezier(0.22,1,0.36,1)' }}
          />

          {/* Side arrows — visible on sm+ */}
          <button
            onClick={onPrev}
          ...
          <div className="p-5 sm:p-8">
            ...
            <div className="flex flex-col gap-0 mb-5">
              {[
                ...
                <div key={label} className="flex justify-between py-3 border-b border-white/8">
                  <span className="text-[#8a8580] text-[11px] uppercase tracking-wider">{label}</span>
                  <span className={`text-[11px] uppercase tracking-wider font-medium ${gold ? 'text-[#c9b99a]' : 'text-[#f5f0eb]'}`}>{value}</span>
                </div>
              ))}
            </div>

            <p className="text-[#8a8580] font-light text-[13px] leading-[1.8] mb-6">
              {artwork.description}
            </p>
            ...
            <a
              href="/#commissions"
              onClick={onClose}
              className="inline-flex items-center gap-2 border border-[#c9b99a]/40 text-[#c9b99a] px-5 py-3 text-[11px] tracking-[0.18em] uppercase font-medium hover:bg-[#c9b99a] hover:text-[#0d0d0d] transition-all duration-300 w-full justify-center"
            >
              Commission Similar →
            </a>
          </div>
...
export default function Gallery() {
  ...
  const prevLightbox  = useCallback(() => setLightboxIndex((i) => (i === null ? null : i === 0 ? filtered.length - 1 : i - 1)), [filtered.length]);
  const nextLightbox  = useCallback(() => setLightboxIndex((i) => (i === null ? null : i === filtered.length - 1 ? 0 : i + 1)), [filtered.length]);
  ...
  return (
    <div className="bg-[#0d0d0d] text-[#f5f0eb] min-h-screen selection:bg-[#c9b99a] selection:text-[#0d0d0d]">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 sm:mb-12">
            ...
            <p className="text-[#8a8580] font-light text-[14px] sm:text-[15px] max-w-[340px] leading-relaxed md:text-right">
              Thirteen works spanning two decades of painting, installation, and performance.
            </p>
          </div>
          ...
          <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap scrollbar-none">
            {FILTERS.map((filter) => (
              ...
              >
                {filter}
                {filter === 'All' && (
                  <span className="ml-1.5 text-[9px] opacity-50">{ALL_ARTWORKS.length}</span>
                )}
              </button>
            ))}
          </div>
      ...
      {/* CTA strip */}
      <div className="border-t border-white/8 py-14 sm:py-20 px-4 sm:px-6 md:px-12">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <h3 className="font-bold text-[clamp(20px,3vw,36px)] text-[#f5f0eb] mb-2">
              Something caught your eye?
            </h3>
            <p className="text-[#8a8580] font-light text-[14px] sm:text-[15px]">
              Commission an original piece or acquire a limited print.
            </p>
          </div>
          <Link
            href="/#commissions"
            className="flex-shrink-0 inline-flex items-center gap-3 bg-[#c9b99a] text-[#0d0d0d] px-6 sm:px-8 py-3.5 sm:py-4 font-semibold text-[12px] sm:text-[13px] tracking-[0.2em] uppercase hover:bg-[#f5f0eb] transition-colors duration-300"
